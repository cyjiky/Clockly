...
class LineChartTasksScheme(VisualizationBase, TimeRangeBase, ModelId, TaskNames, LocationLine):
    visualization_name: str = Field(default="Line Chart Tasks")
    data_instances: List[TaskVisualizationLineChartInstance]
 
class PieChartTasksScheme(VisualizationBase, TimeRangeBase, ModelId, TaskNames):
    visualization_name: str = Field(default="Pie Chart Tasks")
    data_instances: List[TaskVisualizationPieInstance]

class LineChartEventsScheme(VisualizationBase, TimeRangeBase, ModelId, EventsNames, LocationLine):
    visualization_name: str = Field(default="Line Chart Events")
    data_instances: List[EventVisualizationLineChartInstance]

class PieChartEventsScheme(VisualizationBase, TimeRangeBase, ModelId, EventsNames):
    visualization_name: str = Field(default="Line Chart Events")
...
class LineChartScheme(GenericModel, Generic[S]):
    ...
    def check_time_order(self):
        if not isinstance(S, VisualizationBase):
            raise ValueError("Generic visualization type must inherit from VisualizationBase")
        return self
