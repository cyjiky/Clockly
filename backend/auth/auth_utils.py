...
async def authorize_private_endpoint(token: str = Header(..., title="Authorization Bearer token", example="Bearer [token]")) -> Users:
    """
    ...
    try:
        ...
        out_user = await auth_service.authorize_request_jwt_and_return_user(jwt=token)
